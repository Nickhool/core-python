...
<H3>My_Cookie Setting</H3>
<LI> <C0DE><B>CPPuser = %s</B></C0DE>
...
<P><INPUT TYPE=submit>
    ...
    langSet = ('Python', 'Ruby', 'Java', 'Ct+', 'PHP', 'C', 'JavaScript')
    langItem = '<INPUT TYPE=checkbox NAME=lang VALUE-"%s"%s> %s\n'
    def getCPPCookies(self): # reads cookies from client
        if 'HTTP_ COOKIE' in environ:
            cookies = [x.strip() for x in environ['HTTP_CO0KIE'].sp1it(';')]
    ...
    def showForm(self):
        ...
        # see if user cookie_ set up yet
...
Click <A HREF="%s"><B>here</B></A> to return to form.
    ...
    def doResults(self):  # display results page
        ...
        if len(filedata) == MAXBYTES:
            filedata = '%s%s'.encode('UTF-8') % (filedata,
    ...
    def go(self): # determine which page to return
        ...
        if 'person' in form:
            ...
            self.error = 'Your name is required. (mi ssing) '
